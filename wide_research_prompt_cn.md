...
6. **程序化聚合**
   - 使用脚本（如 `aggregate.py`）逐个读取 `child_outputs/` 内的 Markdown 文件：每次读取一份稿件，写入主报告后再处理下一份，避免一次性加载所有内容。
   - 输出主结果文件（建议为 Markdown，例如 `runs/<...>/final_report.md`），并在合适位置嵌入子稿中的引用与结论。
   - 聚合过程中记录来源去重、段落合并策略以及遇到的缺口，确保日志可追溯。
