...
6. **程序化聚合**
   - 使用脚本（如 `aggregate.py`）一次性读取 `child_outputs/` 内的全部 Markdown 文件，并按既定顺序直接拼接成一个完整的主 Markdown 报告。
   - 输出主结果文件（建议为 Markdown，例如 `runs/<...>/final_report.md`），在适当位置保留子稿引用与核心结论，形成单一文档。

7. **最终复核与局部修复**
   - 对聚合结果做 sanity check。
   - 若发现小问题（拼写、字段顺序、缺失 metadata），使用代码对局部段落或章节顺序进行微调；禁止整篇重写或大幅改写子进程内容。
...
- **禁止原生网络命令**：全程不得使用 `wget`、`curl` 等直接联网指令或启用 `sandbox_workspace_write.network_access`；所有外部数据必须通过 MCP 工具获取。
...
- **子进程自治**：在 prompt 中明确要求子进程全程自主执行（避免等待人工确认或 plan 工具阻塞），并给出可直接运行的示例脚本（如 Python 模板或文字处理伪代码），确保其可落地且不依赖人工输入。
...
- **结构化输出优先**：如需在 Markdown 之外保留中间数据，可统一使用 JSON/CSV 等结构化格式，并在 schema 内预留 `status`、`reason` 等字段，以便聚合脚本能对缺失或异常情况稳健降级。
- **缓存与日志并重**：原始内容、清洗文本、执行日志分别缓存到 `raw/`、`tmp/`、`logs/`，让复查与复跑都有可追溯的依据，同时降低重复抓取或解析的成本。
- **输出校验**：子流程生成文件后立即校验语法（Markdown 是否完整、可选的 JSON 是否有效）；若文件损坏则先删除旧文件再重跑该子任务，防止后续聚合被异常内容污染。
- **避免重复抓取**：重试前确认对应 `child_outputs/<id>.md` 是否已经合法存在，合法则跳过，既节省配额也避免重复命中站点。
- **人工复核切入口**：在自动摘要或聚合后，预留可手动修订的层（如可在 Markdown 中插入校验标记或附加脚本检查），保证在长尾网页或复杂语料下能快速介入修正。
...
## 示例
- `scripts/wide_research_example.sh` 展示了从索引缓存、子 Prompt 模板生成、到 `codex exec` 并行调度的整套流程。该脚本先拉取 `https://yage.ai/tag/deepseek.html` 以获知任务规模，再让子代理负责下载、解析与总结，并在末尾校验子输出的 Markdown 报告。
